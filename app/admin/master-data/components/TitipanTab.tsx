...
export default function TitipanTab() {
  ...
  const save = async () => {
    const buy = Number(draft.harga_beli);
    const sell = Number(draft.harga_jual);
    ...
    if (
      ...
      setError("Lengkapi nama, unit, dan harga yang valid.");
      return;
    }
    ...
    const result = editingId
      ? await supabase
          ...
          .eq("id", editingId)
          .select("*")
      : await supabase.from("ms_titipan").insert(payload).select("*");
  ...
  if (loading) {
    return (
      <div className={`rounded-2xl border bg-white p-8 text-center text-sm text-zinc-500 ${midText.sm}`}>
        Memuat data titipan...
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-3 shadow-sm sm:p-4">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 className={`text-xl font-bold ${midText.lg}`}>Titipan</h2>
        <button
          type="button"
          onClick={() => {
            reset();
      ...
      {visibleRows.length === 0 ? (
        ...
        <div className="overflow-x-hidden">
          <table className={`w-full table-fixed text-left text-sm min-[850px]:text-base ${midText.sm}`}>
            <thead className="bg-zinc-50">
              <tr>
                <th className="w-[20%] px-2 py-2 sm:px-3" style={{ fontSize: 0.7 + "rem" }}>Nama (unit)</th>
                <th className="w-[15%] px-2 py-2 sm:px-3" style={{ fontSize: 0.7 + "rem" }}>Harga Beli</th>
                <th className="w-[15%] px-2 py-2 sm:px-3" style={{ fontSize: 0.7 + "rem" }}>Harga Jual</th>
                <th className="w-[15%] px-2 py-2 sm:px-3" style={{ fontSize: 0.7 + "rem" }}>Margin</th>
                <th className="w-[10%] px-2 py-2 sm:px-3" style={{ fontSize: 0.7 + "rem" }}>Status</th>
                <th className="w-[10%] px-2 py-2 sm:px-3" style={{ fontSize: 0.7 + "rem" }}>Aksi</th>
              </tr>
            </thead>
            <tbody>
              {visibleRows.map((row) => (
                <tr key={row.id} className="border-t border-zinc-200 align-middle">
                  <td className="break-words px-2 py-2 sm:px-3" style={{ fontSize: 0.6 + "rem" }}>
                    {row.nama} ({row.unit})
                  </td>
                  <td className="break-words px-2 py-2 sm:px-3" style={{ fontSize: 0.6 + "rem" }}>
                    {money(row.harga_beli)}
                  </td>
                  <td className="break-words px-2 py-2 sm:px-3" style={{ fontSize: 0.6 + "rem" }}>
                    {money(row.harga_jual)}
                  </td>
                  <td className="break-words px-2 py-2 sm:px-3" style={{ fontSize: 0.6 + "rem" }}>
                    {money(Number(row.harga_jual ?? 0) - Number(row.harga_beli ?? 0))}
                  </td>
                  <td className="break-words px-2 py-2 sm:px-3" style={{ fontSize: 0.6 + "rem" }} >
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-2 sm:items-center sm:p-4">
          <div className="max-h-[calc(100dvh-1rem)] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-4 shadow-xl sm:p-6">
            <h3 className="mb-4 text-xl font-bold">
              {editingId ? "Edit Titipan" : "Tambah Titipan"}
            </h3>
            <div className="grid gap-3 sm:grid-cols-2">
              {(["nama", "unit", "harga_beli", "harga_jual"] as const).map((field) => (
                <label key={field} className="text-sm font-medium min-[850px]:text-base">
                  {field.replace("_", " ")}
                  <input
                    type={field.includes("harga") ? "number" : "text"}
                    min={field.includes("harga") ? "0" : undefined}
                    value={draft[field] ?? ""}
              ...
              <label className="text-sm font-medium min-[850px]:text-base" style={{ fontSize: 0.75 + "rem" }}>
                Status
                <select
                  value={draft.aktif ? "aktif" : "nonaktif"}
                  onChange={(e) => setDraft({ ...draft, aktif: e.target.value === "aktif" })}
                  className="mt-1 min-h-11 w-full rounded border px-2 text-sm min-[850px]:text-base"
                >
